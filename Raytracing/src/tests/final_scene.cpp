...
camera init_camera( double aspect_ratio){
...
hittable_list random_scene(){
    ...
    auto ground_material = make_shared<metal>( color( 0.5, 0.5, 0.5), 0.5);
    world.add_object( make_shared<sphere>( point3( 0, -1000, 0), 1000, ground_material) );
    ...
    for (int a = -11; a < 11; a++) {
        for (int b = -11; b < 11; b++) {
            ...
            if ((center - point3(4, 0.2, 0)).length() > 0.9) {
                ...
                world.add_object(make_shared<sphere>(center, 0.2, sphere_material));
            }
        }
    }
    // auto material1 = make_shared<dielectric>(1.5);
    // world.add_object(make_shared<sphere>(point3(0, 1, 0), 1.0, material1));

    // auto material2 = make_shared<lambertian>(color(0.4, 0.2, 0.1));
    // world.add_object(make_shared<sphere>(point3(-4, 1, 0), 1.0, material2));

    // auto material3 = make_shared<metal>(color(0.7, 0.6, 0.5), 0.0);
    // world.add_object(make_shared<sphere>(point3(4, 1, 0), 1.0, material3));
...
int main(){ 
    ...
    camera cam = init_camera(aspect_ratio);
